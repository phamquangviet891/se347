...
namespace se347_be.APIs
{
    public class MyOrder
    {
        ...
        public async Task<bool> addOrder(List<long> list_cart_item_id, long address_id, List<long> vouchers_id)
        {
            ...
            newOrder.vouchers= new List<string>();
            using (DataContext context = new DataContext())
            {//.Include(s => s.product)
                List<SqlCartItem> list_cart_item = context.cart_items.Where(s => list_cart_item_id.Contains(s.ID)).Include(s => s.cart).ThenInclude(s => s.shop).Include(s=>s.product).ToList();
                ...
                List<SqlVoucher> list_voucher = context.vouchers.Where(s => vouchers_id.Contains(s.ID)).ToList();
                if (list_voucher.Any())
                {
                    foreach (SqlVoucher item in list_voucher)
                    {
                        item.list_user_applied.Add(address.user.ID.ToString());
                        string data = JsonConvert.SerializeObject(item);
                        newOrder.vouchers.Add(data);
                        //  không lưu total, nhưng lưu thêm cái list voucher đã áp vào
                    }
                }
