...
namespace se347_be.APIs
{
    public class MyCategory
    {
        ...
        public async Task<bool> added_slug()
        {
            using (DataContext context = new DataContext())
            {
                List<SqlCategory> categories = context.categories.ToList();
                foreach (SqlCategory category in categories)
                {
                    category.slug = DataContext.GetSlug(category.title);
                }
                await context.SaveChangesAsync();
            }
            return true;
        ...
        public async Task<bool> tool_update_product_to_Bitis()
        {
            using (DataContext context = new DataContext())
            {
                ...
                //await context.products.Include(s => s.category).Where(s => s.category.ID > 6 && s.category.ID > 47).ForEachAsync(s=>s.shop=shop);
                List<SqlCategory> categories = context.categories.Where(s => s.ID > 6 && s.ID < 47).Include(s => s.products).ToList();
                foreach (SqlCategory category in categories)
                {
                    foreach (SqlProduct product in category.products)
                    {
                        product.shop = shop;
                    }
                }
