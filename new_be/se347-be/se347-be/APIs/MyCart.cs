...
namespace se347_be.APIs
{
    public class MyCart
    {
        ...
        public async Task<bool> tool_remove_wrong_cart_item()
        {
            using (DataContext context = new DataContext())
            {
                List<SqlCartItem> cart_items = context.cart_items.Where(s => !string.IsNullOrEmpty(s.option)).Include(s => s.product).ToList();
                foreach (SqlCartItem sqlCartItem in cart_items)
                {
                    if (sqlCartItem.product.options.Any())
                    {
                        if (!string.IsNullOrEmpty(sqlCartItem.option))
                        {
                            if (!sqlCartItem.product.options.Contains(sqlCartItem.option)){
                                //sqlCartItem.isDeleted = true;
                                //context.cart_items.Remove(sqlCartItem);
                            }
                        }
